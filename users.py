...
async def get_current_user(security_scopes: SecurityScopes, token: str = Depends(oauth2_scheme), dependencies= Depends(get_db)):
...
async def login_for_access_token(form_data: OAuth2PasswordRequestForm = Depends(), dependencies= Depends(get_db)):
...
async def register(*, user: schemas.UserCreate, dependencies = Depends(get_db), background_tasks: BackgroundTasks):
