...
async def post_dependency_check(post_id: int = Path(default=None), owner: User = Depends(users.get_current_active_user), dependencies=Depends(get_db)):
    post_db = crud.get_post(post_id=post_id)
    if not post_db:
...
async def blog_posts(skip: int = 0, limit: int = 100, dependencies=Depends(get_db)):
    return crud.get_posts(skip=skip, limit=limit)
...
dependencies=Depends(get_db)):
    ...
    return crud.create_user_post(title=title, image=image, description=description, created_at=created_at, user_id=current_user.id)
    # return crud.create_user_post(db, post=post_data, user_id=active_user.id)

# get a blog post
@router.get("/{post_id}", response_model=Post)
async def blog_post(post_id: int = Path(default=None), dependencies=Depends(get_db)):
    post_data = crud.get_post(post_id=post_id)
    if not post_data:
...
def update_post_for_user(*, post_data: Post = Security(post_dependency_check, scopes=["update"]), post: PostUpdate, dependencies=Depends(get_db), current_user: User = Security(users.get_current_user, scopes=["update"])):
    return crud.update_user_post(payload=post, post_data=post_data)


@router.delete("/{post_id}")
def delete_posts(post_data: Post = Security(post_dependency_check, scopes=["delete"]), dependencies=Depends(get_db)):
    crud.delete_post(post_data=post_data)
